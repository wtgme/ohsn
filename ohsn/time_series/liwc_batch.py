...
def compare_periods(filename):
    ...
    print names
    noi = ['posemo', 'health', 'body', 'negemo', 'ingest', 'death']
    ...
    for name in noi:
        ...
        active_time = []
        for user in users:
            uvs = df[(df.user_id == user)].loc[:, ['time_index', name, 'user_created_time', 'last_tweet_time']]
            # print uvs
            if len(uvs) == 2:
                # print user
                old = uvs.iloc[0][name]
                new = uvs.iloc[1][name]
                period = (uvs.iloc[1]['last_tweet_time'] - uvs.iloc[1]['user_created_time']).days
                # print period
                if old != 0:
                    change = (new - old)
                    # print change
        ...
        # pltt.correlation(changes, active_time, r'$\Delta$('+name+')', 'Active Time', name+'activetime-cor-recovery.pdf')
