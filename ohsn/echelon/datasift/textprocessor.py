...
MIN_RESOLUTION_TIMELINE = datetime.timedelta(seconds=86400)
MIN_RESOLUTION_FOLLOWER = datetime.timedelta(seconds=1*604800)
...
def identifyDatasiftRetweet(tweet):
# if its an official retweet ignore the tweet itself
    try:
...
def create_poi_from_datasift_tweet(ds_tweet, poi):
    ...
    newpoi['descriptions'] = []
    newpoi['poi_classification'] = 0
    newpoi['datetime_last_timeline_scrape'] = datetime.datetime.now() - MIN_RESOLUTION_TIMELINE
    newpoi['datetime_last_follower_scrape'] = datetime.datetime.now() - MIN_RESOLUTION_FOLLOWER
    # newpoi['datetime_last_friend_scrape'] = datetime.datetime.now() - MIN_RESOLUTION_FOLLOWER
    newpoi['timeline_auth_error_flag']  = False
    newpoi['follower_auth_error_flag']  = False
    newpoi['timeline_latest_id'] = None
    # newpoi['friend_auth_error_flag']  = False
...
#        and we need to embed the obsevations behind a tag on the poi
   ...
   # if its an official retweet ignore the tweet itself
    if not (identifyDatasiftRetweet(tweet)):
