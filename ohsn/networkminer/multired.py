    ...
    def draw_dendrogram(self, force = False):
        ...
        if np.all(self.Z) == None:
            if not force:
                ...
                self.reduce()
        zscore = [0] + self.Z[:,2].tolist()
        qv = self.q_vals
        print zscore
        print qv
        f, (ax1, ax2) = plt.subplots(2, 1,sharex=True)
        ax1.plot(zscore, qv,ls='-', marker='o')
        ax1.set_ylabel(r'$q$')
        ax2 = dendrogram(self.Z, no_plot=False, labels=["Mental", "Social", "Fitness", "Body", "Thinspo"], orientation='right', color_threshold=0)
        # ax1.set_ylabel('q')
        matplotlib.pyplot.draw()
        matplotlib.pyplot.show()
        matplotlib.pyplot.savefig('merger.pdf')
