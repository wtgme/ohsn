...
def tweet_rate(start, end, username, rate):
    ...
    while end > period_end:
        ...
        the_count = tweets.find({'created_at': {'$gte': period_start, '$lt': period_end},'user.screen_name': username}).count()    
        # the_count = 0        
...
for username in usernames:
    period_count = tweet_rate(start, end, username, rate)
    plt.plot_date(x=period_count[0], y=period_count[1], fmt=usernames[username], label=username)
